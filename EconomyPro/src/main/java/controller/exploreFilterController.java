...
public class exploreFilterController implements Controller {
	...
	public String requestHandler(HttpServletRequest request, HttpServletResponse response)
		...
		String[] platList = request.getParameterValues("PLAT");
		...
		String gen = request.getParameter("themes");
		if(gen==null) {
			gen = "액션";
		}
		...
		// 상기 배열 형식의 platList를 하기와 같이 쿼리용 정규표현식으로 바꿔준다. '~~', '~~', ...
		if(platList ==null) {
			//만약에 아무 checkbox도 선택하지 않았을 경우 모든 플랫폼 선택한 것으로 넘겨준다.
			stringList = "'NV01','KK01','LZ01','TT01','TM01',";
		} else {
			for(int i =0; i<platList.length; i++) {
				stringList += "'";
				stringList += platList[i];
				stringList += "'";
				stringList += ",";
			}
			
		}
		System.out.println(stringList.substring(0,stringList.length()-1));
		System.out.println(point);
		System.out.println(gen);
		// "'NV01','KK01','LZ01','TT01','TM01'," 이런식으로 마지막에 ,는 쿼리문에서 불필요 하므로 substring으로 지워준다.
		// vo 형식 중 plat_id를 위의 가공된 string으로 넣어준다.
		vo.setPlat_id(stringList.substring(0,stringList.length()-1));
		...
		List<WebtoonVO> list =  dao.filtListWT(vo);
